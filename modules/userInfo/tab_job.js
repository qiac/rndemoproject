...
import JobListItem from './tab_job_item';

class TabJobComp extends Component {
  constructor(props) {
    super(props);
  ...
  refresh(finishCallback) {
    ...
    HttpUtils.get(Service.JOB_LIST_URL,
      function(response) {
        if(response) {
          // 刷新列表
  ...
  renderItem(item: json, sectionID: number, rowID: number) {
    ...
    var props = {
      store_thumb: item.poster_info.cover,
